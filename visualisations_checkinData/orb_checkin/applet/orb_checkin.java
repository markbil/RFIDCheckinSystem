...
public class orb_checkin extends PApplet {
...
int c1 = color(102, 102, 0);
int c2 = color(255, 0, 0);
int c3 = color(0, 255, 0);
int c4 = color(0, 0, 255);
int c5 = color(127, 127, 127);
...
public void setup () { // Setup, everything is set up before the code actually runs.
  ...
  int daysSinceCheckin;
    try {
      ...
      for (int i = 0; i < checkins_length; i++){
        ...
        daysSinceCheckin = PApplet.parseInt(checkin.getString("days_since_checkin"));        
...
class Particle { // The particle class, holds all the data and formulas for each particle.
  ...
  Particle (int c) { // Particle constructor. This runs every time a new particle is created
    distance = random(100,200); // The distance is set randomly between 100 and 200
    theta = random(6.28f); // The longways angle is randomly set
    phi = random(3.14f); // The sideways angle is randomly set
    this.c = c; 
  ...
  public void update (int number) { // Update the particle. Its number is used as a parameter to tell how far it is from the camera in relation to the other particles.
    phi += (250 - distance) * 0.00017f; // Increase the phi (sideways angle) according to its distance from the center. Particles farther out are faster.
    ...
    if (colored) { // If boolean colored is true, we color the particles different hues, according to their angle from the center.
      ...
      stroke(50 * (1 + cos(angle)), 100, (550 - number) * 0.2f); // The stroke and fill colors are determined by the angle, and the brightness is determined by their distance from the camera
      fill(50 * (1 + cos(angle)), 100, (550 - number) * 0.2f);
    }
...
//      fill((550 - number) * 0.14);    
    ...
    ellipse (location.x, location.y, (1600- (this.added() + 1000)) * 0.04f, (1600- (this.added() + 1000)) * 0.04f); // Draw the particle.
...
public Particle[] zBuffer (Particle[] particles) {
...
public void fastBlur(PImage img, int radius) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "orb_checkin" });
  }
}
