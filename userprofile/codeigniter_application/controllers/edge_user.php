...
class Edge_user extends CI_Controller {
	...
	{
		//validate form input
		$this->edge_common->form_validation->set_rules('identity', 'Identity', 'required');
		$this->edge_common->form_validation->set_rules('password', 'Password', 'required');

		if ($this->edge_common->form_validation->run() == true)
		{ //check to see if the user is logging in
			//check for "remember me"
			$remember = (bool) $this->input->post('remember');

			if ($this->edge_common->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{ //if the login is successful
				//redirect them back to the home page
				$this->edge_common->session->set_flashdata('message', $this->edge_common->ion_auth->messages());
				redirect('edge_user/profile', 'refresh');
			} else	{ //if the login was un-successful
				//redirect them back to the login page
				$this->edge_common->session->set_flashdata('message', $this->edge_common->ion_auth->errors());
		...
		{  
			...
			$page_data['message'] = (validation_errors()) ? validation_errors() : $this->edge_common->session->flashdata('message');
			$page_data['identity'] = array('name' => 'identity',
				'id' => 'identity',
				'type' => 'text',
				'value' => $this->edge_common->form_validation->set_value('identity'),
			);
			...
			$this->edge_common->render_page('edge_user/index', $page_data);
		}
		}
	}
	...
	function profile($user_id=null) {
		...
		$page_data['allow_access'] = ($user_id != $this->edge_common->session->userdata('user_id') && $this->edge_common->ion_auth->is_admin()) || $user_id == $this->edge_common->session->userdata('user_id');
		if($page_data['allow_access']) {
			$page_data['title'] = "Your Profile Details";
			$page_data['message']=$this->edge_common->session->flashdata('message');
	...
	function feedback() {		
		if($this->edge_common->ion_auth->logged_in()) {
			if($user_id=$this->edge_common->session->userdata('user_id')) {
				$this->edge_common->form_validation->set_rules('feedback_comment', 'Feedback', 'required');
				$this->edge_common->form_validation->set_message('feedback_comment', 'Your Feedback in the Field Below is Required!');
				
				if ($this->edge_common->form_validation->run() === FALSE) {			
...
//					$page_data['user_id'] = $user_id;
				...
				$this->edge_common->render_page('edge_user/feedback', $page_data);	
			}
		} else {
			$this->edge_common->render_page('edge_user/login', 'refresh');		
		}
	}
	...
	function logout()
	{
		...
		//log the user out
		$logout = $this->edge_common->ion_auth->logout();
	...
	{
		$this->edge_common->form_validation->set_rules('old', 'Old password', 'required');
		$this->edge_common->form_validation->set_rules('new', 'New Password', 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[new_confirm]');
		$this->edge_common->form_validation->set_rules('new_confirm', 'Confirm New Password', 'required');
		...
		$user = $this->edge_common->ion_auth->user()->row();
		...
		{ //display the form
			...
			$this->edge_common->render_page('edge_user/change_password', $page_data);
		}
		else
		{
			$identity = $this->edge_common->session->userdata($this->config->item('identity', 'ion_auth'));

			$change = $this->edge_common->ion_auth->change_password($identity, $this->input->post('old'), $this->input->post('new'));

			if ($change)
			{ //if the password was successfully changed
				//$this->edge_common->session->set_flashdata('message', $this->edge_common->ion_auth->messages());
				$this->edge_common->session->set_flashdata('message', "Password Changed !");
				//$this->logout();
				redirect('edge_user/profile', 'refresh');
			}
			else
			{
				//$this->edge_common->session->set_flashdata('message', $this->edge_common->ion_auth->errors());
				$this->edge_common->session->set_flashdata('message', "Error in changing Password !");
	...
	{
		$this->edge_common->form_validation->set_rules('email', 'Email Address', 'required');
		if ($this->edge_common->form_validation->run() == false)
		{
			//setup the input
			...
			$page_data['message'] = (validation_errors()) ? validation_errors() : $this->edge_common->session->flashdata('message');
			$this->edge_common->render_page('edge_user/forgot_password', $page_data);
		}
		else
		{
			//run the forgotten password method to email an activation code to the user
			$forgotten = $this->edge_common->ion_auth->forgotten_password($this->input->post('email'));
	...
	public function reset_password($code = NULL)
	{
		...
		$user = $this->edge_common->ion_auth->forgotten_password_check($code);
		...
		{  //if the code is valid then display the password reset form
			...
			{//display the form
				...
				//render
				$this->edge_common->render_page('edge_user/reset_password', $page_data);
			}
			else
			{
				...
				if ($this->_valid_csrf_nonce() === FALSE || $user->id != $this->input->post('user_id')) {
					...
					$this->edge_common->ion_auth->clear_forgotten_password_code($code);
					...
					$change = $this->edge_common->ion_auth->reset_password($identity, $this->input->post('new'));
	...
	{
		if ($code !== false)
			$activation = $this->edge_common->ion_auth->activate($id, $code);
		else if ($this->edge_common->ion_auth->is_admin())
			$activation = $this->edge_common->ion_auth->activate($id);
	...
	function deactivate($id = NULL)
	{
		...
		$this->edge_common->form_validation->set_rules('confirm', 'confirmation', 'required');
		$this->edge_common->form_validation->set_rules('id', 'user ID', 'required|alpha_numeric');

		if ($this->edge_common->form_validation->run() == FALSE)
		{
			// insert csrf check
			$page_data['csrf'] = $this->_get_csrf_nonce();
			$page_data['user'] = $this->edge_common->ion_auth->user($id)->row();

			$this->edge_common->render_page('edge_user/deactivate_user', $page_data);
		}
		else
		{
			...
			if ($this->input->post('confirm') == 'yes')
			{
				...
				if ($this->edge_common->ion_auth->logged_in() && $this->edge_common->ion_auth->is_admin())
				{
					$this->edge_common->ion_auth->deactivate($id);
				}
			}
	...
	*/
	public function signup()
	{
		...
		$this->edge_common->form_validation->set_rules('username', 'User Name', 'required|min_length[5]|max_length[15]|is_unique[edge_users.username]');
		$this->edge_common->form_validation->set_rules('password', 'Password', 'required|matches[password_confirm]|min_length[8]');
		$this->edge_common->form_validation->set_rules('password_confirm', 'Password Confirmation', 'required|min_length[8]');

		if ($this->edge_common->form_validation->run() === FALSE)
		{
			$this->edge_common->render_page('edge_user/signup', $page_data);
		}
		else
		{
	...
	public function update($user_id=null){
		...
		$this->edge_common->form_validation->set_rules('user_profile_rfid_ID', 'Swipe Card ID', 'callback_validate_rfid');
		$this->edge_common->form_validation->set_rules('user_profile_rfid', 'Swipe Card ID', 'callback_validate_rfid');
		$this->edge_common->form_validation->set_rules('username', 'User Name', 'required|min_length[5]');
		$this->edge_common->form_validation->set_rules('firstname', 'First Name', 'required');
		$this->edge_common->form_validation->set_rules('lastname', 'Last Name', 'required');
		$this->edge_common->form_validation->set_rules('email', 'Email Address', 'required|valid_email|min_length[9]');
	//	$this->edge_common->form_validation->set_rules('occupation', 'Occupation', 'required|max_length[25]');
		if ( strlen($this->input->post('new_interest'))) {
			$this->edge_common->form_validation->set_rules('new_interest', 'Interest Term', 'min_length[3]');
			$this->edge_common->form_validation->set_rules('new_interest_level', 'Interest Level', 'greater_than[0]|less_than[6]');
		}
			if ( strlen($this->input->post('new_expertise'))) {
			$this->edge_common->form_validation->set_rules('new_expertise', 'Expertise Term', 'min_length[3]');
			$this->edge_common->form_validation->set_rules('new_expertise_level', 'Expertise Level', 'greater_than[0]|less_than[6]');
		}
		...
		$page_data['user_details'] = array(
				...
				//$this->edge_common->ion_auth->in_group($this->config->item('admin_group', 'ion_auth'), $user_id),
	...
	function create()
	{
		...
		//validate form input
		$this->edge_common->form_validation->set_rules('first_name', 'First Name', 'required|xss_clean');
		$this->edge_common->form_validation->set_rules('last_name', 'Last Name', 'required|xss_clean');
		$this->edge_common->form_validation->set_rules('email', 'Email Address', 'required|valid_email');
		$this->edge_common->form_validation->set_rules('password', 'Password', 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[password_confirm]');
		$this->edge_common->form_validation->set_rules('password_confirm', 'Password Confirmation', 'required');
		...
		if ($this->edge_common->form_validation->run() == true)
		{
			...
			$new_userid=$this->edge_common->ion_auth->register($username, $password, $email, $additional_data, array());
			if($new_userid) {
				//redirect them back to the admin page
				$this->edge_common->session->set_flashdata('message', 'New User Added!');
		...
		{ //display the create user form
			//set the flash data error message if there is one
			$page_data['message'] = (validation_errors() ? validation_errors() : ($this->edge_common->ion_auth->errors() ? $this->edge_common->ion_auth->errors() : $this->edge_common->session->flashdata('message')));

			$page_data['first_name'] = array('name' => 'first_name',
				'id' => 'first_name',
				'type' => 'text',
				'value' => $this->edge_common->form_validation->set_value('first_name'),
			);
			$page_data['last_name'] = array('name' => 'last_name',
				'id' => 'last_name',
				'type' => 'text',
				'value' => $this->edge_common->form_validation->set_value('last_name'),
			);
			$page_data['email'] = array('name' => 'email',
				'id' => 'email',
				'type' => 'text',
				'value' => $this->edge_common->form_validation->set_value('email'),
			);
			$page_data['password'] = array('name' => 'password',
				'id' => 'password',
				'type' => 'password',
				'value' => $this->edge_common->form_validation->set_value('password'),
			);
	...
	function list_users() {
		...
		//list the users
		$page_data['users'] = $this->edge_common->ion_auth->users()->result();
		foreach ($page_data['users'] as $k => $user)
		{
			$page_data['users'][$k]->groups = $this->edge_common->ion_auth->get_users_groups($user->id)->result();
		}
		
		$this->edge_common->render_page('edge_user/list', $page_data);		
	}
	...
	function _get_csrf_nonce()
	{
		...
		$this->edge_common->session->set_flashdata('csrfkey', $key);
		$this->edge_common->session->set_flashdata('csrfvalue', $value);
	...
	{
		if ($this->input->post($this->edge_common->session->flashdata('csrfkey')) !== FALSE &&
				$this->input->post($this->edge_common->session->flashdata('csrfkey')) == $this->edge_common->session->flashdata('csrfvalue'))
		{
			return TRUE;
		}
		else
	...
	private function  _get_groups() {
		$groups_result=$this->edge_common->ion_auth->groups();
		$groups = array();
